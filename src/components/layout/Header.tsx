...
const Header: React.FC = () => {
  ...
  return (
    <>
      {/* Contact Bar - Only show on desktop and when not scrolled */}
      <AnimatePresence>
        {!isScrolled && (
          <motion.div
            ...
            className="hidden lg:block bg-primary-900 text-white py-2 relative z-50"
          >
              ...
              <div className="flex justify-between items-center text-sm">
                <div className="flex items-center space-x-6">
                  ...
                  <a 
                    ...
                    className="flex items-center gap-2 hover:text-accent-300 transition-colors"
                    onClick={() => trackEvent('Contact', 'email_click', 'header')}
                  >
          ...
          >
            {navLinks.map((link) => (
              <Link
                ...
                className={`font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-400 rounded-md px-2 py-1 ${
                  isScrolled 
            ...
            {/* Mobile Contact Info - Show when scrolled */}
            {isScrolled && (
              <a 
                href="tel:+17165551234"
                className="hidden md:flex items-center gap-2 text-primary-600 hover:text-primary-700 transition-colors text-sm font-medium"
                onClick={() => trackEvent('Contact', 'phone_click', 'header_scrolled')}
              >
                <Phone className="w-4 h-4" />
                (716) 555-1234
              </a>
              ...
              >
                ...
                <div className="p-4 bg-primary-50 border-b">
                  <div className="space-y-3">
                    <a 
                      href="tel:+17165551234"
                      className="flex items-center gap-3 text-primary-900 font-medium"
                ...
                <div className="flex-1 flex flex-col justify-center space-y-8 px-4">
                  {navLinks.map((link) => (
                    ...
                    >
                      <Link
                        to={link.path}
                        className="text-gray-800 text-2xl font-medium hover:text-primary-600 focus-visible:text-primary-600 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-400 rounded-md px-2 py-1 block"
                        onClick={() => handleNavClick(link.name.toLowerCase())}
                      >
                        {link.name}
                      </Link>
